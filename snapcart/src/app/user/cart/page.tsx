    ...
    <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
      ...
      {cartData.length == 0 ?
        ...
        (
          ...
          >
            <ShoppingBasket className='h-16 w-16 text-gray-400 mx-auto mb-4'/>
            <p className='text-gray-600 text-lg mb-6'>Your Cart is empty. Add some groceries to continue shopping!</p>
            <Link href={'/'} className='bg-green-600 text-white px-6 py-3 rounded-full hover:bg-green-700 transition-full inline-block font-medium'>Continue Shopping</Link>
          </motion.div>
        )
        :
          ...
          <div className='lg:col-span-2 space-y-5'>
            <AnimatePresence>
              {cartData.map((item,index)=>(
                ...
                >
               <div className='relative w-28 h-28 sm:w-24 sm:h-24 md:h-28 md:w-28 shrink-0 rounded-xloverflow-hidden bg-gray-50'>
                <Image 
                src={item.image} 
                alt={item.name}
                fill
              ...
              {/* button for increase and decrease item qunatity */}
               ...
               <button className='sm:ml-4 mt-3 sm:mt-0 text-red-500 hover:text-red-700 transition-all '><Trash2 size={18} onClick={()=>dispatch(removeFromCart(item._id))} /></button>
