// Client component for rendering admin dashboard with animations and charts
'use client'
...
function AdminDashboardClient({ earning ,stats, chartData}: propType) {
  // State to track selected time filter for earnings display
  const [filter, setFilter] = useState<"today" | "sevenDays" | "total">()

  // Calculate current earning based on selected filter
  const currentEarning = filter === "today" ? earning.today
    : filter === "sevenDays" ? earning.sevenDays
      : earning.total

  // Set title text based on selected filter
  ...
  return (
    <div className='pt-28 w-[90%] md:w-[80%] mx-auto'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10 text-center sm:text-left'>
        ...
        {/* options for filtering earnings */}
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10'>
        {stats.map((s,i)=>{
          // Icon array matching the stats order
          const icons=[
            <Package key="p" className='text-green-700 h-6 w-6'/>,
            <Users key="u" className='text-green-700 h-6 w-6'/>,
            <Truck key="t" className='text-green-700 h-6 w-6'/>,
            <IndianRupee key="i" className='text-green-700 h-6 w-6'/>,
          ]
          return <motion.div
          key={i}
          initial={{ opacity: 0, y: 20 }}
...
{/* for showing chart we have to use recharts library "npm install recharts" */}
      {/* Chart section - displays 7-day orders overview using recharts */}
      <div className='bg-white border border-gray-100 rounded-2xl shadow-md p-5 mb-10'>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartData}>
            <CartesianGrid stroke='#ccc' strokeDasharray='5 5' /> {/* Grid background */}
            <XAxis dataKey='day' /> {/* X-axis showing day names */}
            <Tooltip /> {/* Hover tooltip */}
            <Bar dataKey='orders' fill='#16A34A' radius={[6,6,0,0]} /> {/* Green bars with rounded tops */}
            </BarChart>
          </ResponsiveContainer>
      </div>
    </div>
