...
const RegisterForm = ({previousStep}:propType) => {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative '>
      ...
      <p className='flex text-gray-600 mb-8 items-center'>Join snapCart Today <LeafIcon className='h-5 w-5 text-green-700'/></p>
    ...
    <div  className='w-full flex items-center justify-center gap-3 border-gray-300 hover:bg-gray-50 py-3 rounded-xl text-gray-700 font-medium transition-all duration-200' onClick={()=>signIn("google",{callbackUrl:"/"})}> 
      ...
      <p className=' cursor-pointer flex text-gray-600 mt-6 text-sm items-center gap-1' onClick={()=>router.push('/login')}>Already have an account ? <LogIn className='h-4 w-4' />
      <span className='text-green-500'>Sign in</span></p>
